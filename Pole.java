...
public class Pole {
	...
	public void attract(Pole opposite) {
    	...
    	if (location.distanceTo(opposite.getLocation()) < ATTRACTION_RANGE) {
        	...
        	if (Math.abs(xoff) >= Math.abs(yoff)) {
            	...
            	if (xoff < 0) {
                	opposite.getParent().moveAll(-enclosingMagnet.getWidth(), 0);
            	} else {
                	opposite.getParent().moveAll(enclosingMagnet.getWidth(), 0);
            	}
        	} else {
            	...
            	opposite.getParent().moveAll(location.getX() - opposite.getLocation().getX(), 0);
            	if (yoff < 0) {
                	opposite.getParent().moveAll(0, -enclosingMagnet.getHeight());
            	} else {
                	opposite.getParent().moveAll(0, enclosingMagnet.getHeight());
            	}
	...
	public void repel(Pole similar) {
    	...
    	if (distance < ATTRACTION_RANGE - 1) {
        	...
        	similar.getParent().moveAll(xoff, yoff);
    	}
